...
import { getPracticeAreasForSolution } from "@/data/practice-areas";
...
const practiceAreas = getPracticeAreasForSolution(SOLUTION_ID);
      ...
      <div className="h-full overflow-y-auto">
        <div className="mx-auto max-w-6xl space-y-8 px-6 py-6">
          ...
          {practiceAreas.length > 0 && (
            <section>
              <h3 className="mb-3 text-[16px] font-semibold text-foreground">Practice areas</h3>
              <div className="space-y-6">
                {practiceAreas.map((area) => (
                  <PracticeAreaSection key={area.id} practiceArea={area} />
                ))}
              </div>
            </section>
          )}
