...
export function PartnerBuilderItem({ item, onRemove, onUpdateNote }: Props) {
  const partner = PARTNERS.find((p) => p.id === item.id);
  const [noteOpen, setNoteOpen] = useState(!!item.note);
  if (!partner) return null;

  const relevantProducts = partner.productIds
    ...
    <div className="rounded-lg border border-border bg-card p-4">
      <div className="flex items-start justify-between gap-3">
        <div className="flex-1 min-w-0">
          <span className="text-[10px] font-semibold uppercase tracking-wide text-foreground/50">Partner</span>
          <p className="mt-0.5 text-sm font-semibold text-foreground">{partner.name}</p>
          <p className="text-xs text-muted-foreground">{partner.category}</p>
          <p className="mt-1.5 text-xs text-muted-foreground line-clamp-2 leading-relaxed">{partner.description}</p>
          {relevantProducts.length > 0 && (
            <div className="mt-2 flex flex-wrap gap-1">
              {relevantProducts.map((label) => (
                <span key={label} className="rounded bg-secondary px-1.5 py-0.5 text-[10px] text-foreground/70">{label}</span>
              ))}
            </div>
          )}
        </div>
